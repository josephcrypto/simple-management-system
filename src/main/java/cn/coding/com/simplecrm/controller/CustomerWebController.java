...
public class CustomerWebController {
    ...
    @RequestMapping(value = "data_for_datatable", produces = "application/json", method = RequestMethod.GET)
    @ResponseBody
    public String getDataForDatatable(@RequestParam Map<String, Object> params) {
        ...
        String dataTableOrderColumnName = "column[" + dataTableOrderColumnIdx + "][data]";
        ...
        Page<Customer> customers = customerService.getCustomerForDatatable(queryString, pageRequest);
        ...
        customers.forEach(customer -> {
            ...
            cellData.put("emailAddress", customer.getAddress());
    ...
    @PostMapping("/save")
    public String save(@Valid @ModelAttribute("customerInstance") Customer customerInstance, BindingResult bindingResult,
                       Model model, RedirectAttributes atts) {
        if (bindingResult.hasErrors()) {
            return "/customer/create.html";
        } else  {
            if (customerRepository.save(customerInstance) != null)
                ...
                atts.addFlashAttribute("message", "Customer create Failed.");
